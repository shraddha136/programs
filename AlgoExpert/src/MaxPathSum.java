...
class Solution2 {
    ...
    public static int maxPathSum(TreeNode tree) {
        // Write your code here.
        Sum sum = getSum(tree);
        return sum.runningMaxSum;
    }

    private static Sum getSum(TreeNode root) {
        if (root == null)
            return new Sum();
        Sum lefty = getSum(root.left);
        Sum righty = getSum(root.right);
        ...
        int maxSumAsBranch = Math.max(maxChildSumAsBranch + val, val);
        int maxSumAsRoot = Math.max(maxSumAsBranch, lefty.maxSumAsBranch + val + righty.maxSumAsBranch);
        int maxPathSum = Math.max(Math.max(lefty.runningMaxSum, righty.runningMaxSum), maxSumAsRoot);
